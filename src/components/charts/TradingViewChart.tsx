  ...
  if (showUniswapInfo && liquidityEvents.liquidityEvents.length > 0) {
    ...
    return (
      <div className="w-full h-[500px] bg-gray-800 rounded-lg overflow-hidden flex flex-col items-center justify-center p-6">
        ...
        <h2 className="text-lg font-bold text-white mb-2">{tokenInfo.name} Listed on Blazeswap</h2>
        <br/>
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="text-center">
            <p className="text-sm text-subtext">Token</p>
            <p className="text-lg font-semibold text-white">{formatAmountV3(event.tokenAmount)} {tokenInfo.symbol}</p>
          </div>
          <div className="text-center">
            <p className="text-sm text-subtext">$AIMG</p>
            <p className="text-lg font-semibold text-white">{formatAmountV3(event.ethAmount)} $AIMG</p>
          </div>
        </div>
        <div className="flex space-x-4">
          <a
            href={`${process.env.NEXT_PUBLIC_ETHERSCAN_URL}/tx/${event.txHash}`}
            target="_blank"
          ...
          <a
            href={`https://app.blazeswap.xyz/swap/?outputCurrency=${tokenInfo.address}`}
            target="_blank"
            rel="noopener noreferrer"
