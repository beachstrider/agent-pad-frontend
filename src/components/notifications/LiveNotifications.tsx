...
const LiveNotifications: React.FC = () => {
  ...
  const createNotificationMessage = (data: { type: string; data: any }): Notification => {
    ...
    switch (data.type) {
      case 'buy':
        return {
          message: `${addressEnd} Bought ${formatAmountV2(data.data.ethAmount)} $AIMG of ${data.data.symbol}`,
          type: 'buy',
          logo: data.data.logo
        };
      case 'sell':
        return {
          message: `${addressEnd} Sold ${formatAmountV2(data.data.ethAmount)} $AIMG of ${data.data.symbol}`,
          type: 'sell',
          logo: data.data.logo
        };
      case 'tokenCreated':
  ...
  return (
    <div className="bg-[var(--card)] text-white py-1 overflow-hidden sticky top-0 z-50 border-b border-[var(--card)]">
      <div ref={containerRef} className="flex whitespace-nowrap items-center">
        <div className="flex items-center space-x-2 mx-3">
          <span className="text-xs font-medium text-subtext">{currentNotification.message}</span>
