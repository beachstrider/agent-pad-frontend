...
export async function getTokenHolders(tokenAddress: string): Promise<TokenHolder[]> {
  try {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_BLOCKSCOUT_URL}/api/v2/tokens/${tokenAddress}/holders`);
...
//get all token holders count
export async function getTokenHoldersCount(tokenAddress: string): Promise<{ token_holders_count: number }> {
  try {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_BLOCKSCOUT_URL}/api/v2/tokens/${tokenAddress}/counters`);
    console.log('total holder count:', response.data.token_holders_count);
    return response.data;
  } catch (error) {
    console.error('Error fetching token holders count:', error);
    throw new Error('Failed to fetch token holders count');
  }
}
